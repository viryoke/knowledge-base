...
def fetch_transcript(video_id: str, languages: list = None):
    ...
    # v1.0+ API: create instance, use fetch() method
...
def main():
    ...
    try:
        ...
        elif "blocking" in error_msg.lower() or "ip" in error_msg.lower():
            print(json.dumps({"error": "YouTube is blocking requests. Try yt-dlp with --cookies-from-browser chrome"}))
        else:
